...
if (isset($_POST['rent_room'])) {
    ...
    $start_date = $_POST['start_date']; // Get the start date from the form
    ...
    if ($result && $result->num_rows > 0) {
        ...
        $stmt = $mysqli->prepare("INSERT INTO rentals (student_number, room_id, start_date) VALUES (?, ?, ?)");
        $stmt->bind_param("sis", $student_number, $room_id, $start_date); // Bind start_date as well
        $stmt->execute();
...
$reservations_result = $mysqli->query("SELECT rooms.room_number, rooms.capacity, rooms.room_type, rentals.start_date FROM rentals JOIN rooms ON rentals.room_id = rooms.id WHERE rentals.student_number = '{$_SESSION['student_number']}'");
...
<head>
    ...
    <title>Dorm Management Dashboard</title>
...
<body id="body-pd">
    ...
    <div class="l-navbar" id="nav-bar">
        <nav class="nav">
            <div>
                ...
                <div class="nav_list"> 
                    <a href="renting.php" class="nav_link active"> <i class='bx bx-grid-alt nav_icon'></i> <span class="nav_name">Dashboard</span> </a>
    ...
    <div class="height-100 bg-light">
        ...
        <p>Your student number is: <?php echo htmlspecialchars($_SESSION['student_number']); ?></p>
            ...
            <tbody>
                <?php while ($reservation = $reservations_result->fetch_assoc()): ?>
                    <tr>
                        ...
                        <td><?php echo htmlspecialchars($reservation['start_date']); ?></td> <!-- Display start date -->
                    </tr>
                <?php endwhile; ?>
            </tbody>
        </table>
